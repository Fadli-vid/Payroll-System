...
export default function AllowancesPage() {
  ...
  const handleSubmit = async (values: AllowanceFormValues) => {
    try {
      if (editingId) {
        await axios.put(`/api/allowances/${editingId}`, values);
        toast.success("Tunjangan berhasil diperbarui");
      } else {
        await axios.post("/api/allowances", values);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">Data Tunjangan</h2>
        <p className="text-muted-foreground">
          Kelola master data tunjangan (allowance) karyawan.
        </p>
      </div>

      <Card>
        <CardHeader className="pb-4">
          <CardTitle>Daftar Tunjangan</CardTitle>
          <CardDescription>
            Tunjangan akan ditambahkan ke gaji pokok karyawan setiap bulan.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable
            ...
            searchPlaceholder="Cari tunjangan..."
            actions={
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {editingId ? "Edit Tunjangan" : "Tambah Tunjangan"}
            </DialogTitle>
            <DialogDescription>
              {editingId
                ? "Perbarui informasi tunjangan."
                : "Tambahkan tunjangan baru ke master data."}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name">
                Nama Tunjangan <span className="text-destructive">*</span>
              </Label>
              <Input
                id="name"
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="description"
                placeholder="Penjelasan singkat mengenai tunjangan ini..."
